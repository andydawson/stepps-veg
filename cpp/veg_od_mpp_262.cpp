...
namespace veg_pp {
  ...
  public:
	...
	throw std::runtime_error("variable eta missing");
      ...
      pos__ = 0U;
      context__.validate_dims("initialization", "eta", "vector_d", context__.to_vec(W));
      vector_d eta(W);
	...
	throw std::runtime_error("variable rho missing");
      ...
      pos__ = 0U;
      context__.validate_dims("initialization", "rho", "vector_d", context__.to_vec(W));
      vector_d rho(W);
      for (int j1__ = 0U; j1__ < W; ++j1__)
	...
	throw std::runtime_error(std::string("Error transforming variable mu: ") + e.what());
        ...
        pos__ = 0U;
        context__.validate_dims("initialization", "alpha", "vector_d", context__.to_vec(W,N_knots));
        std::vector<vector_d> alpha(W,vector_d(N_knots));
        ...
        pos__ = 0U;
        context__.validate_dims("initialization", "g", "vector_d", context__.to_vec(W,N));
        std::vector<vector_d> g(W,vector_d(N));
   ...
   void transform_inits(const stan::io::var_context& context,
    ...
    double log_prob_grad(vector<double>& params_r,
      ...
      double lp = 0.0;
      paleon::Timer timer_total(1), timer_dss(W), timer_dgc(W), timer_exp(W), timer_inv(W), timer_drho(W);
      ...
      vector_d eta(W), eta_lja(W), eta_ja(W), eta_dj(W);
      for (int i=0; i<W; i++)
      ...
      vector_d rho(W), rho_lja(W), rho_ja(W), rho_dj(W);
      for (int i=0; i<W; i++)
      ...
      vector<matrix_d> C(W);
      vector<matrix_d> c(W);
      vector<matrix_d> ct(W);
      vector<Eigen::LLT<matrix_d> > llt(W);

      matrix_d exp_g(N,W);
      matrix_d r(N,K);

      vector<matrix_d> C_inv(W);
      vector<vector_d> mu_g(W);
      vector<vector_d> sigma2(W);
      vector<vector_d> ci_Cinv_ci(W);
      vector<vector_d> cCalpha(W);
	...
	lp += lp_thrd[k];
      ...
      for (int i = 0; i < N; ++i) {
        ...
        if (r.row(i).sum() > 0.0)
          lp += multinomial_log_double(y[i], r.row(i));
      }
	...
	for (int i=0; i<N; i++) {
	  ...
	  // g normal
	  gradient[3*W+W*N_knots+k*N+i] -= gn;

	  // rho normal
	  gradient[W+k] += -0.5 * gc * (2 * dgcdrho[i] * ss - gc * dssdrho) * ss2inv * rho_ja[k];
	  gradient[W+k] += -0.5 * ssinv * dssdrho * rho_ja[k];

	  for (int j=0; j<N_knots; j++)
	    gradient[3*W+k*N_knots+j] += gn * m_c_Cinv(j,i);
	...
	for (int m=0; m<K; m++) {
	  for (int i=0; i<N; i++) {
	    ...
	    gradient[3*W+W*N_knots+k*N+i] += y[i][m] / r(i,m) * drdg;
          }
        }
      }
    ...
    void write_array(RNG& base_rng__,
      ...
      vector_d eta = in__.vector_lub_constrain(0.10000000000000001,sqrt(100),W);
      vector_d rho = in__.vector_lub_constrain(1.0e-6,1.0,W);
